...
//    PORT_AUTH_FAIL = 0x02,
  eRS485ErrorCode    eStatus = RS485_ENOERR;
	...
	if(pucBuffer[0] == CHECK_STATUS) // need to redefine offset: function code
  {
    if(CTport.Port[ucPort + RS485_PORT_OFFSET].active == PORT_ENABLE)
    {
      pucBuffer[1] = 0x00;	//need to redefine offset: port status
    }
    else
    {
      pucBuffer[1] = 0x01;
  ...
  else if((pucBuffer[0] == CHECK_UID_AUTHENTICATION)|(pucBuffer[0] == CHECK_KEY_AUTHENTICATION)) //  need to redefine offset: function code
  {
    if(CTport.Port[ucPort + RS485_PORT_OFFSET].event & PORT_TRANSMIT_DATA) // colaborate with mqtt task
    {
      ...
      {
        pucBuffer[1] = 0x01;	//need to redefine offset: port status
      }
      CTport.Port[ucPort + RS485_PORT_OFFSET].event = PORT_EVENT_NONE;	//need to redefine offset: port status
    }
    else
    {
       pucBuffer[1] = 0x02; 	//need to redefine offset: port status
    }
  }
	// we need add reponse funciton via rs485 modbus
	*ucLen = 2;
  return eStatus;
}
