...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress) {
	...
	assert_param(
			IS_ETH_RETRY_TRANSMISSION(ETH_InitStruct->ETH_RetryTransmission));
	assert_param(
			IS_ETH_AUTOMATIC_PADCRC_STRIP(ETH_InitStruct->ETH_AutomaticPadCRCStrip));
	...
	assert_param(
			IS_ETH_SOURCE_ADDR_FILTER(ETH_InitStruct->ETH_SourceAddrFilter));
	assert_param(IS_ETH_CONTROL_FRAMES(ETH_InitStruct->ETH_PassControlFrames));
	assert_param(
			IS_ETH_BROADCAST_FRAMES_RECEPTION(ETH_InitStruct->ETH_BroadcastFramesReception));
	assert_param(
			IS_ETH_DESTINATION_ADDR_FILTER(ETH_InitStruct->ETH_DestinationAddrFilter));
	assert_param(IS_ETH_PROMISCIOUS_MODE(ETH_InitStruct->ETH_PromiscuousMode));
	assert_param(
			IS_ETH_MULTICAST_FRAMES_FILTER(ETH_InitStruct->ETH_MulticastFramesFilter));
	assert_param(
			IS_ETH_UNICAST_FRAMES_FILTER(ETH_InitStruct->ETH_UnicastFramesFilter));
	...
	assert_param(
			IS_ETH_PAUSE_LOW_THRESHOLD(ETH_InitStruct->ETH_PauseLowThreshold));
	assert_param(
			IS_ETH_UNICAST_PAUSE_FRAME_DETECT(ETH_InitStruct->ETH_UnicastPauseFrameDetect));
	assert_param(
			IS_ETH_RECEIVE_FLOWCONTROL(ETH_InitStruct->ETH_ReceiveFlowControl));
	assert_param(
			IS_ETH_TRANSMIT_FLOWCONTROL(ETH_InitStruct->ETH_TransmitFlowControl));
	assert_param(
			IS_ETH_VLAN_TAG_COMPARISON(ETH_InitStruct->ETH_VLANTagComparison));
	assert_param(
			IS_ETH_VLAN_TAG_IDENTIFIER(ETH_InitStruct->ETH_VLANTagIdentifier));
	/* DMA --------------------------*/
	assert_param(
			IS_ETH_DROP_TCPIP_CHECKSUM_FRAME(ETH_InitStruct->ETH_DropTCPIPChecksumErrorFrame));
	assert_param(
			IS_ETH_RECEIVE_STORE_FORWARD(ETH_InitStruct->ETH_ReceiveStoreForward));
	assert_param(
			IS_ETH_FLUSH_RECEIVE_FRAME(ETH_InitStruct->ETH_FlushReceivedFrame));
	assert_param(
			IS_ETH_TRANSMIT_STORE_FORWARD(ETH_InitStruct->ETH_TransmitStoreForward));
	assert_param(
			IS_ETH_TRANSMIT_THRESHOLD_CONTROL(ETH_InitStruct->ETH_TransmitThresholdControl));
	assert_param(
			IS_ETH_FORWARD_ERROR_FRAMES(ETH_InitStruct->ETH_ForwardErrorFrames));
	assert_param(
			IS_ETH_FORWARD_UNDERSIZED_GOOD_FRAMES(ETH_InitStruct->ETH_ForwardUndersizedGoodFrames));
	assert_param(
			IS_ETH_RECEIVE_THRESHOLD_CONTROL(ETH_InitStruct->ETH_ReceiveThresholdControl));
	assert_param(
			IS_ETH_SECOND_FRAME_OPERATE(ETH_InitStruct->ETH_SecondFrameOperate));
	assert_param(
			IS_ETH_ADDRESS_ALIGNED_BEATS(ETH_InitStruct->ETH_AddressAlignedBeats));
	assert_param(IS_ETH_FIXED_BURST(ETH_InitStruct->ETH_FixedBurst));
	assert_param(
			IS_ETH_RXDMA_BURST_LENGTH(ETH_InitStruct->ETH_RxDMABurstLength));
	assert_param(
			IS_ETH_TXDMA_BURST_LENGTH(ETH_InitStruct->ETH_TxDMABurstLength));
	assert_param(
			IS_ETH_DMA_DESC_SKIP_LENGTH(ETH_InitStruct->ETH_DescriptorSkipLength));
	assert_param(
			IS_ETH_DMA_ARBITRATION_ROUNDROBIN_RXTX(ETH_InitStruct->ETH_DMAArbitration));
	/*-------------------------------- MAC Config ------------------------------*/
	...
	} else if ((hclk >= 100000000) && (hclk <= 168000000)) {
...
//	_eth_delay_(0x1FFFFF);
	...
	do {
		timeout++;
		auxiliar = ETH_ReadPHYRegister(PHYAddress, PHY_BSR);
	} while (!(auxiliar & PHY_Linked_Status)
			&& (timeout < PHY_READ_TO));
	...
	if (ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable) {
		...
		do{
			...
			RegValue = ETH_ReadPHYRegister(PHYAddress, ((uint16_t)29));
			RegValue = ETH_ReadPHYRegister(PHYAddress, PHY_SR);
			RegValue&= PHY_DUPLEX_SPEED_STATUS_MASK;
		...
		if (!ETH_WritePHYRegister(PHYAddress, PHY_BCR,
				((uint16_t)(ETH_InitStruct->ETH_Mode >> 3)
...
//		auxiliar = ETH_ReadPHYRegister(PHYAddress, PHY_BCR);
//		_eth_delay_(PHY_RESET_DELAY);
//		auxiliar = ETH_ReadPHYRegister(PHYAddress, 29);
//		_eth_delay_(PHY_RESET_DELAY);
//		auxiliar = ETH_ReadPHYRegister(PHYAddress, PHY_SR);
//		_eth_delay_(PHY_RESET_DELAY);
	...
	tmpreg |= (uint32_t)(
			ETH_InitStruct->ETH_Watchdog | ETH_InitStruct->ETH_Jabber
	...
	/* Write to ETHERNET MACFFR */ETH->MACFFR = (uint32_t)(
	...
	/* Write to ETHERNET MACHTHR */ETH->MACHTHR =
			(uint32_t) ETH_InitStruct->ETH_HashTableHigh;
	/* Write to ETHERNET MACHTLR */ETH->MACHTLR =
	...
	tmpreg |= (uint32_t)(
			(ETH_InitStruct->ETH_PauseTime << 16)
					| ETH_InitStruct->ETH_ZeroQuantaPause
					| ETH_InitStruct->ETH_PauseLowThreshold
					| ETH_InitStruct->ETH_UnicastPauseFrameDetect
					| ETH_InitStruct->ETH_ReceiveFlowControl
					| ETH_InitStruct->ETH_TransmitFlowControl);
	/* Write to ETHERNET MACFCR */ETH->MACFCR = (uint32_t) tmpreg;
	...
	/* Write to ETHERNET DMAOMR */ETH->DMAOMR = (uint32_t) tmpreg;
	...
	/* Set the PR and DA bits according to ETH_DMAArbitration value */ETH->DMABMR =
			(uint32_t)(
	...
	/* Enable the Enhanced DMA descriptors */ETH->DMABMR |= ETH_DMABMR_EDE;
...
FrameTypeDef ETH_Get_Received_Frame_interrupt(void) {
	...
	while (((DMARxDescToGet->Status & ETH_DMARxDesc_OWN) == (uint32_t) RESET)
		...
		else if (((DMARxDescToGet->Status & ETH_DMARxDesc_LS)
				== (uint32_t) RESET)
...
uint32_t ETH_Prepare_Transmit_Descriptors(u16 FrameLength) {
	...
	__IO
	ETH_DMADESCTypeDef *DMATxNextDesc;
	...
	if (buf_count == 1) {
		...
		for (i = 0; i < buf_count; i++) {
			...
			/* Program size */
			DMATxNextDesc->ControlBufferSize = (ETH_TX_BUF_SIZE
					& ETH_DMATxDesc_TBS1);
			...
			DMATxNextDesc =
					(ETH_DMADESCTypeDef *) (DMATxNextDesc->Buffer2NextDescAddr);
			/* Set Own bit of the Tx descriptor Status: gives the buffer back to ETHERNET DMA */
		}
...
void ETH_DMARxDescChainInit(ETH_DMADESCTypeDef *DMARxDescTab, uint8_t *RxBuff,
	...
	/* Set Receive Descriptor List Address Register */ETH->DMARDLAR =
 ...
 */
FlagStatus ETH_GetDMAPTPRxDescExtendedFlagStatus(
		ETH_DMADESCTypeDef *DMAPTPRxDesc, uint32_t ETH_DMAPTPRxDescExtendedFlag) {
	FlagStatus bitstatus = RESET;

	/* Check the parameters */
	assert_param(
			IS_ETH_DMAPTPRxDESC_GET_EXTENDED_FLAG(ETH_DMAPTPRxDescExtendedFlag));
...
uint32_t ETH_GetRxOverflowMissedFrameCounter(void) {
	return ((uint32_t)(
			(ETH->DMAMFBOCR & ETH_DMAMFBOCR_MFA)
...
uint16_t ETH_ReadPHYRegister(uint16_t PHYAddress, uint16_t PHYReg) {
	...
	/* Write the result value into the MII Address register */ETH->MACMIIAR =
			tmpreg;
...
uint32_t ETH_WritePHYRegister(uint16_t PHYAddress, uint16_t PHYReg,
	...
	tmpreg |= ETH_MACMIIAR_MB; /* Set the MII Busy bit */
	/* Give the value to the MII data register */ETH->MACMIIDR = PHYValue;
	/* Write the result value into the MII Address register */ETH->MACMIIAR =
			tmpreg;
	/* Check for the Busy flag */
	do {
