...
vMBPortSerialEnable(BOOL xRxEnable, BOOL xTxEnable )
{
	...
	eEvent = EVENT_SERIAL_TRANS_START;
	if( xRxEnable )
	{
	...
	else
	{
		...
		USART_ITConfig(UartPortGet(0),USART_IT_RXNE,DISABLE);
...
xMBPortSerialInit( ULONG ulBaudRate, UCHAR ucDataBits, eMBParity eParity )
{
	...
	/* UARTx clock enable */
  UART_RCC_Configuration(UartPortGet(0));
	
	/* Configure GPIO as alternate function push-pull */
  UART_GPIO_Configuration(UartPortGet(0));
	
  /* Configure Pin RDE*/
  RS485_RDE_GPIO_Configuration(0);
  
	/* Enable the USART1 interrupt. */
	NVIC_InitStructure.NVIC_IRQChannel = IRQPortGet(0);
  ...
  USART_Init(UartPortGet(0), &USART_InitStructure);
...
xMBPortSerialPutByte(CHAR ucByte )
{
  EventSerialType eEvent;
	 ...
	 * called. */
	USART_SendData(UartPortGet(0), ucByte);
  while(!USART_GetFlagStatus(UartPortGet(0),USART_FLAG_TC)){};
...
xMBPortSerialGetByte( CHAR * pucByte )
{
	 ...
	 */
	*pucByte = (CHAR) USART_ReceiveData(UartPortGet(0));
   return TRUE;
}

/* ----------------------- Close Serial Port ----------------------------------*/
void
vPortSerialClose( void )
{
  NVIC_InitTypeDef   NVIC_InitStructure;
	NVIC_InitStructure.NVIC_IRQChannel = IRQPortGet(0);
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 6;
	...
	NVIC_Init(&NVIC_InitStructure);
	USART_Cmd(UartPortGet(0), DISABLE);
}
