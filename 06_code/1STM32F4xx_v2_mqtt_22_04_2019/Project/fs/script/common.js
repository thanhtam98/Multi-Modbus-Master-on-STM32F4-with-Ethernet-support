...
function CheckNetmask(id) {
  if (false == CheckIpAddr(id)) {
    return false;
  }
  var nip = IP2Int($(id).value);
  var zero = true;
  for (var i = 0; i < 32; i++) {
    if (zero == false) {
      if ((nip & (1 << i)) == 0) {
...
function samesubnet(ip, mask) {
  ...
  if (sip.length != 4 || smask.length != 4) {
    return false;
  }
  if (
    parseInt(sip[0]) != (parseInt(sip[0]) & parseInt(smask[0])) ||
    parseInt(sip[1]) != (parseInt(sip[1]) & parseInt(smask[1])) ||
    parseInt(sip[2]) != (parseInt(sip[2]) & parseInt(smask[2])) ||
    parseInt(sip[3]) != (parseInt(sip[3]) & parseInt(smask[3]))
  ) {
    return false;
  } else {
    return true;
