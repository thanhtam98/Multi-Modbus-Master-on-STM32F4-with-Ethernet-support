...
BOOL xMBTCPPortInit(USHORT usTCPPort)
{
    ...
    // sAddr.sin_port = usTCPPort;
    // sAddr.sin_addr.s_addr = host.IP.idata

    // if( ( pxPCBListenNew = pxPCBListenOld = socket(AF_INET,SOCK_STREAM, IPPROTO_TCP) <0 )
    // {
    //     /* Can't create TCP socket. */
    //     bOkay = FALSE;
    // }
    // if ((retVal = connect(pxPCBListenNew, (struct sockaddr*)&sAddr, sizeof(sAddr))) < 0)
    // {
    // 	close(n->my_socket);
    //     goto exit;
    // }

    // else if(retVal = connect(pxPCBListenNew, (struct sockaddr*)&sAddr, sizeof(sAddr))) < 0)
    // {
    //     /* Bind failed - Maybe illegal port value or in use. */
    //     close(pxPCBListenNew);
    //     bOkay = FALSE;
    // }
    ...
    {
        // don't use callback because of rtos implementation
    ...
    else
    {
        ...
        if (usTCPBufPos >= MB_TCP_FUNC)
        {
            ...
            {	
                           ...
                           "Received to few bytes! Droping client.\r\n");
            }
...
#endif
                (void)xMBPortEventPost(TCP_EV_FRAME_RECEIVED);
            }
            else
            {
#ifdef MB_TCP_DEBUG
