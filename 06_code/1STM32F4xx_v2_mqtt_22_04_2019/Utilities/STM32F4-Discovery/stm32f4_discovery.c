...
/** @defgroup STM32F4_DISCOVERY_LOW_LEVEL_Private_Variables
  * @{
  */ 
GPIO_TypeDef* GPIO_PORT[LEDn] = {LED0_GPIO_PORT, LED1_GPIO_PORT, LED2_GPIO_PORT, LED3_GPIO_PORT, TEST_GPIO_PORT};
const uint16_t GPIO_PIN[LEDn] = {LED0_PIN, LED1_PIN, LED2_PIN, LED3_PIN, TEST_PIN};
const uint32_t GPIO_CLK[LEDn] = {LED0_GPIO_CLK, LED1_GPIO_CLK, LED2_GPIO_CLK, LED3_GPIO_CLK, TEST_GPIO_CLK};
...
/**
  * @brief  Configures Buzzer Controll GPIO.
  * @param  None
  * @retval None
  */
...
void STM_EVAL_OutCTLInit(void)
{
	GPIO_InitTypeDef  GPIO_InitStructure;
	
	/* Enable the OUTPUT CONTROLL Clock */
	RCC_AHB1PeriphClockCmd(OUTPUT_CTL_GPIO_CLK, ENABLE);
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT; 
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Pin = OUTPUT_CTL_PIN;
	/* Configure the OUTPUT CONTROLL pin */
	GPIO_Init(OUTPUT_CTL_GPIO_PORT, &GPIO_InitStructure);
	STM_Output_LOW();
}

/**
  * @brief  set Output Controll Low.
  * @param  None
  * @retval None
  */
void STM_Output_LOW(void)
{
	GPIO_ResetBits(OUTPUT_CTL_GPIO_PORT,OUTPUT_CTL_PIN);
}
/**
  * @brief  set Output Controll High.
  * @param  None
  * @retval None
  */
void STM_Output_HIGH(void)
{
	GPIO_SetBits(OUTPUT_CTL_GPIO_PORT,OUTPUT_CTL_PIN);
}

/**
  * @brief  set Buzzer off.
  * @param  None
  * @retval None
  */
void STM_Buzzer_OFF(void)
{
...
/**
  ...
  * @retval None
  */
void STM_Buzzer_ON(void)
{
  DBG("\r\nBuzzer is buzzingggggg");
